...
if [ $start_flag -le 4 -a $end_flag -ge 4 ]; then
	...
	if [ $rflag = "FSL" ]; then
		...
		if [ $q_flag = "yes" ];then
			antsRegistrationSyNQuick.sh -d 3 -f ${tpldir}/avg152T1_brain.nii.gz -m ${workdir}/step02/MR2mni_${t1}_Warped.nii* -o ${workdir}/step04/NLR_${t1}_ -t so -n 1 
			...
			antsRegistrationSyN.sh -d 3 -f ${tpldir}/avg152T1_brain.nii.gz -m ${workdir}/step02/MR2mni_${t1}_Warped.nii* -o ${workdir}/step04/NLR_${t1}_ -t so -n 1 
			WarpImageMultiTransform 3 ${workdir}/step03/PET2MR_${pet}_Warped.nii.gz ${workdir}/step04/NLR_${pet}.nii.gz -R ${workdir}/step04/NLR_${t1}_Warped.nii.gz ${workdir}/step04/NLR_${t1}_1Warp.nii.gz 
		else 
			echo "Unexpected arguments : --Quick = "$q_flag ; exit 0
		fi
